...
| `char(n)` / `varchar(n)` / `string`(`text`) | `CHAR(n)`/`VARCHAR(n)`/`VARCHAR` (unbounded) | `getString` | **mode-dependent** (`doris.string-pushdown.mode`, default GUARDED) | G5 RESOLVED by live probe (REPORT-string-comparison-probe-4.1.3): Doris 4.1.3 string compare/IN/BETWEEN/LIKE/ORDER BY are pure byte semantics (`utf8mb4_0900_bin`) == Trino VARCHAR. Modes: NULL_ONLY = old behavior; GUARDED (default) = varchar/string domains as superset pre-filter with Trino filter RETAINED (0x00-bearing values skipped, CHAR excluded — trailing-space stored data under-returns undetectably); BINARY/FULL = full pushdown + VARCHAR TopN + LIKE (backslash-doubled; ESCAPE passthrough); CHAR sort keys never pushed. |
